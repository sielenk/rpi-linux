...
static int socket = 0;
module_param(socket, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(socket, "Choose socket /dev/spidev-0.<x>.");
...
/*
	...
	dev_dbg(&ts->spi->dev,
		" %s ier=0x%02x iir=0x%02x msr=0x%02x lsr=0x%02x\n", __func__,
		ier, chan->iir, chan->msr, chan->lsr);
/*
...
static int __devinit sc16is7x2_probe(struct spi_device *spi)
{
	...
	ret = gpio_request_one(gpio_irq, GPIOF_IN, "SC16IS7x2 irq");
	if (ret < 0) {
		printk(KERN_ERR "Unable to allocate GPIO%d (IRQ)\n", gpio_irq);

		goto exit_destroy;
	}
	spi->irq = gpio_to_irq(gpio_irq);
	ts->gpio_irq = gpio_irq;
	...
	dev_info(&spi->dev, DRIVER_NAME " at CS%d (irq %d), 2 UARTs, 8 GPIOs\n"
		 ...
		 pdata->uart_base, pdata->uart_base + 1, pdata->gpio_base);
...
static int __init sc16is7x2_init(void)
{
	...
	case 1:
		break;
	default:
		printk(KERN_ERR "socket=%d is invalid\n", socket);
	...
	snprintf(buff, sizeof(buff), "%s.%u",
		 dev_name(&spi_device->master->dev), socket);
